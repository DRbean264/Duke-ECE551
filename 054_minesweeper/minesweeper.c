...
int isOutBoard(const board_t *board, int x, int y) {
...
int countMines(board_t * b, int x, int y) {
    ...
    for (int i = -1; i < 2; ++i) {
        for (int j = -1; j < 2; ++j) {
            ...
            if (!isOutBoard(b, x + i, y + j) && IS_MINE(b->board[y + j][x + i])) {  //  if inside the board and the grid has a mine in it
                ++count;
            }
        }
    }
...
void freeBoard(board_t * b) {
    //WRITE ME!
    //  first free all the rows, and the col array, then the struct
    for (int i = 0; i < b->height; ++i) {
        free(b->board[i]);
    }
    free(b->board);
